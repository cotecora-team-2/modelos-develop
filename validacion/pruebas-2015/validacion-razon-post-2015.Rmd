...
author: "Cotecora equipo 2 - estimador de razón combinado"
date: "4/16/2021"
...
data_stratum <- marco_2015 %>% count(estrato_df)
est_completa_tbl <- map(reps, function(rep){
  ...
  fit_estimates <- ratio_estimation(muestra, stratum = estrato_df,
                      data_stratum = data_stratum,
                      n_stratum = n, 
                      parties = all_of(partidos),
                      std_errors = TRUE, B = 200)
  est <- fit_estimates
  est <- mutate(est, inf = prop / 100 - 2 * std_error / 100,
                     sup = prop / 100 + 2 * std_error / 100)
  est$rep <- rep
  est
})
```
