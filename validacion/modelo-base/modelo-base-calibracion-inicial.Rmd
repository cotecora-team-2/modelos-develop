...
```{r}
datos_ent <- conteo_2018 %>% filter(NOMBRE_ESTADO == "CHIHUAHUA") %>% 
...
sim_datos_lista$in_sample <- datos_ent$in_sample
...
```{r}
num_draw <- 229
obtener_muestra <- function(num_draw, seed = 1){
  sims_1 <- sims_casillas %>% filter(.draw == num_draw)
  muestra_1 <- quickcountmx::select_sample_prop(sims_1, stratum = estrato_df, frac = 0.04, 
                                                seed = seed)
...
datos_muestra <- obtener_muestra(num_draw, seed = 23)
```
...
ajuste <- modelo$sample(data = datos_1, 
                          seed = 2211,
                          iter_sampling = 500, iter_warmup = 1200,
                          refresh = 250, 
                          chains = 4, 
                          adapt_delta = 0.95,
                          parallel_chains = 4, 
...
ggplot(y_sims, aes(x = y_out/valores_sim$y_out[1] - 1)) +
  geom_histogram() +
