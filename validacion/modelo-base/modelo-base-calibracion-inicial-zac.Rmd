...
```{r}
num_draw <- 121
datos_muestra <- obtener_muestra(sims_casillas, num_draw, frac = 0.15, seed = 1132)
...
ajustes_ensemble <- 
  map(1:150, ~ ajustar_diagnosticos(sims_casillas, .x, frac = 0.15,
        modelo = modelo, 
  ...
  bind_rows()
  write_rds(ajustes_ensemble, "salidas/ensemble_ajustes_in_zac.rds")
} else {
  ajustes_ensemble <- read_rds("salidas/ensemble_ajustes_in_zac.rds")
}
```
