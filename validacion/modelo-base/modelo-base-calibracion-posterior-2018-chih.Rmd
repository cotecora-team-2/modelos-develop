...
candidato <- "AMLO"
muestra_1 <- obtener_muestra_marco(datos_ent, frac = 0.06, seed = 52711)
...
```{r, message=FALSE, warning=FALSE}
ajuste$summary(variables = c("y_out", "beta_0", "beta", "phi_inv"))
total
```
...
if(FALSE){
  ...
  muestra_1 <- obtener_muestra_marco(datos_ent, frac = 0.06)
  sim_datos$y_f <- datos_ent[[candidato]]
  muestra_1$y <- muestra_1[["votos"]][[candidato]] 
  datos_1 <- c(parametros, muestra_1, sim_datos)
  ...
  write_rds(ajuste_muestras_2, "salidas/ajuste-posterior-muestras-2-chih.rds")
} else {
  ajuste_muestras_2 <- read_rds("salidas/ajuste-posterior-muestras-2-chih.rds")
}
```
