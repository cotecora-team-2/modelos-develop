...
$$y_{i} \sim \textrm{NegBin} \left( n_{i}\theta_i, \frac{n_i\theta_i}{\phi_{s(i)}} \right)$$

la parametrización de la binomial negativa que seleccionamos es en la que

- El valor esperado de $y_i$ es $n_i\theta_i$
- La varianza es $n_i\theta_i\left(1 + {\phi_{s(i)}}\right)$
...
$$\phi_s \sim \textrm{Gamma}(`r parametros$phi_inv_param[1]`, `r parametros$phi_inv_param[2]`)$$
Recordamos que la varianza es
$$n_i\theta_i\left(1 + \phi_{s(i)}\right)$$
