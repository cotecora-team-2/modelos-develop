...
En este documento describimos los resultados de un ajuste propuesta para la estimación
de porcentaje de votos para candidatos bajo muestras censuradas por el tiempo. 

Como discutimos en [presentaciones anteriores](https://sim-llegadas-3e5e7b.netlify.app/), la muestras censuradas por el tiempo típicamente no pueden considerarse como submuestras aleatorias de la muestra diseñada y
presentan sesgos potenciales en la estimaciones que resultan en intervalos
que pueden estar lejos de cumplir la cobertura nominal de 95%.





## Primeros resultados de cobertura

 Los datos de cobertura con y sin ajuste de sesgo (usando 100 muestras) se muestran
a continuación:
...
El incremento en tamaño de los intervalos producidos al introducir la corrección se muestra en la siguiente gráfica:
...
Las coberturas resumidas con y sin ajuste son (cobertura media
y precisión máxima):
...
tabla_con <- resumen %>% 
  ...
  summarise(precision_corregida = round(100*max(precision),1), 
            cobertura_corregida = round(mean(cobertura),2),
    .groups = "drop") 
tabla_sin <- resumen %>% 
  ...
  group_by(prop_obs, estado) %>% 
    filter(partidos %in% c("AMLO", "JAMK", "RAC")) %>% 
  summarise(precision = round(100*max(precision),1), 
            cobertura = round(mean(cobertura),2),
    .groups = "drop")
tabla <- left_join(tabla_con, tabla_sin)
```
...
En [este documento](https://sim-llegadas-3e5e7b.netlify.app/) describimos el tamaño y patrón de sesgo producido por muestras censuradas en la elección presidencial de 2018, considerando sólo los estados
con elección de gobernador en 2021. 

En este documento se muestra (con un análisis exploratorio basado en simulaciones de tiempos de llegada y censura de acuerdo a esos tiempos) que
el sesgo cuando hemos observado una proporción $p_{obs}$ de la muestra completa, cuando 
buscamos estimar una proporción  $\theta$, es proporcional a
$$\theta(1-\theta)(1-p_{obs})$$
Aunque la constante de proporcionalidad varía de estado a estado, encontramos que en los peores casos la constante es de alrededor de
$\alpha = 1/5$
...
$$\theta'_i = h^{-1}(\beta_0 + \beta_{st} + x_i^t\beta + w)$$
...
$$s = \frac{1 - p_{obs}}{10}.$$
Dada la naturaleza aproximada de este análisis, podemos decidir la constante de proporcionalidad con datos de de elecciones anteriores, en lugar de usar los argumentos que acabamos de mostrar.
...
g_2 <- ggplot(resumen_tiempos_tbl, aes(x = prop_obs, y = mediana,
  ...
  geom_line() + geom_point() +
  geom_ribbon(alpha =0.5, size = 0) +
  labs(subtitle = "Con corrección por sesgo (Chihuahua), 1/10") +
  ylab("")
```

```{r, echo=FALSE, fig.height=4.5, fig.width=7}
g_1 + g_2 +  plot_layout(guides = 'collect')  & theme(legend.position = 'bottom')
```

## Apéndice

Con un ajuste más grande de $s=(1-p_{obs})/7$, por ejemplo, obtenemos 

```{r, message=FALSE, echo=FALSE}
archivos_con_ajuste_7 <- list.files(path = "resumenes/estados_presentacion_fbias7/", pattern = "*.csv", full.names = TRUE)
con_ajuste_tbl <- map_df(archivos_con_ajuste_7, function(archivo){
  datos <- read_csv(archivo)
  datos
}) %>% filter(partidos!="CAND_IND_01") %>% 
  mutate(tipo = "con ajuste")
